      ...
      {posts.length > 0 ? (
        <div className="my-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          {posts.map((post) => (
            <div
              key={post.slug}
              className="block p-6 border rounded-sm"
            >
              ...
              <a
                href={post.slug}
                className="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-[#1678d3] hover:bg-yellow-300 hover:text-neutral-800 rounded-sm focus:ring-4 focus:outline-none focus:ring-blue-300"
              >
                {t.readMore}
